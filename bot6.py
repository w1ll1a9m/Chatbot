...
GENDER, EVENT, LOCATION, DATE, TIME, CONFLOCATION, CONFDATE, CONFTIME, MEDICAL, ADVICE, POLICE, FINAL, FINAL2, HTYPE  = range(14)
...
def event(update, context):
    ...
    results = mm.finale(evento['text'],10, 0.5);
    ...
    context.user_data['cdate'] = 1
    context.user_data['clocation'] = 1
    context.user_data['ctime'] = 1
    ...
    if results.iloc[0]['Harassment_flg'] == 2:
        Harassment = 'yes'
        update.message.reply_text('I understand \n ')
...
def location(update, context):
    ...
    context.user_data['Date'] = Date
    context.user_data['Time'] = Time
    context.user_data['clocation'] = context.user_data['clocation'] + 1
    
    if context.user_data['clocation'] > 3:
        
        update.message.reply_text('thank you for the information')
        if context.user_data['Date']=='':
            update.message.reply_text('on what date did it happen?',reply_markup=ReplyKeyboardRemove())
            return DATE
        else:
            reply_keyboard2 = [['yes', 'no']]
            update.message.reply_text('so it happened ... ')
...
def date(update, context):
    ...
    context.user_data['Date'] = Date
    context.user_data['Time'] = Time
    context.user_data['cdate'] = context.user_data['clocation'] + 1
    
    if context.user_data['cdate'] > 3:
...
def time(update, context):
    ...
    context.user_data['Date'] = Date
    context.user_data['Time'] = Time
    context.user_data['ctime'] = context.user_data['ctime'] + 1
    
    if context.user_data['ctime'] > 3:
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            HTYPE:[MessageHandler(Filters.text, htype, pass_user_data=True)]
